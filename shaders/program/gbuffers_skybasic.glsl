        ...
        color.rgb += GetShootingStars(starCoord, VdotU, VdotS);
            ...
            if (absVdotS > sunSizeFactor1) {
                ...
                if (VdotS > 0.0) {
                    ...
                    color.rgb = mix(color.rgb, vec3(0.9, 0.5, 0.3) * 15.0, sunMoonMixer);
                } else {
                    ...
                    #if BLOOD_MOON > 0
                        moonNoiseIntensity = mix(1.0, 1.5, getBloodMoon(sunVisibility));
                        moonColor = mix(moonColor, vec3(0.3686, 0.0353, 0.0353), getBloodMoon(sunVisibility));
                    #endif
